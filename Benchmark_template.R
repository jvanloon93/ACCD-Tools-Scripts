...
MSA1_Pop <- MSA_Pop%>%
  subset(GEOID == MSA_Fips[1])

MSA2_Pop <- MSA_Pop%>%
  subset(GEOID == MSA_Fips[2])
...
MSA1_EdTotal<- All_Ed_Total %>%
  subset(GEOID == MSA_Fips[1]) 

MSA2_EdTotal<- All_Ed_Total %>%
  subset(GEOID == MSA_Fips[2]) 
...
MSA1_HigherEd  <- MSA_HigherEd %>%
  subset(GEOID == MSA_Fips[1]) 

MSA2_HigherEd  <- MSA_HigherEd %>%
  subset(GEOID == MSA_Fips[2]) 

#Boredom but you get the pattern for next time
